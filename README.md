...
<!-- test阶段用的metric是KL/W距离。关于这里的具体实现有点疑惑：我们其实是知道误差项的真分布的(比如`c1*coef.c[1] + c2*coef.c[2]+ error`)，感觉其实可以用这个丢进KL/W里作为test set的metric，是不是更合理一些。
而且原代码里在生成完$y_\mathrm{true}=X\beta$之后又额外加了一个正态误差`r2`，再拿去做随机数生成，感觉是不是不太对（疑惑），回头仔细想一下 -->
